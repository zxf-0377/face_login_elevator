# -*- coding:utf-8 -*-
"""
将用户登录时同组中所有128维矩阵向量与用户登录所用图片的向量进行比较，计算出欧氏距离，
找出欧氏距离最小的返回
"""
import numpy as np
import face_mysql
...
class matrix:
    ...
    def get_socres(self, A, ugroup):
        ...
        for i in range(0, len(results), maxlen):
            ...
            pic_socores = self.EuclideanDistances(A, np.array(pic_vectors))  #比较两个128向量的欧氏距离
        ...
        # print(pic_min_names)
        return pic_min_scores.tolist(), pic_min_names, pic_min_uid,pic_min_floor
