# -*- coding:utf-8 -*-
"""
图像识别主程序，利用tensorflow与facenet来识别人脸，并调用face_mysql将人脸生成的张量和图片其他信息存放在数据库中。
同时将这个张量信息存放在json文件中。
"""
import os
import json
...
class face_reconition:
    ...
    # 将一个文件夹下的所有图片转化为json并存入数据库，只能传入文件夹
    def images_to_vectors(self, inpath, outjson_path, modelpath):
        results = dict()

        with tf.Graph().as_default():
            with tf.Session() as sess:
                # 建立facenet embeddings模型
                src.facenet.load_model(modelpath)  # 载入facenet提供的人脸识别模型
                # 获取tensorflow输入:placeholder
